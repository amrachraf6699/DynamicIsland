...
<x-admin.layout title="لوحة التحكم">
    ...
    <section class="grid gap-3 sm:gap-4 lg:gap-6 lg:grid-cols-3">
        <!-- Welcome Card -->
        <div class="glass-panel rounded-xl sm:rounded-2xl lg:rounded-3xl border border-indigo-100 p-4 sm:p-6 shadow-lg shadow-indigo-500/10">
            <p class="text-xs tracking-[0.3em] text-indigo-500">اليوم</p>
            <h2 class="mt-2 text-2xl sm:text-3xl font-bold text-slate-900 text-balance">{{ now()->locale('ar')->translatedFormat('l d F') }}</h2>
            <p class="mt-4 sm:mt-6 text-xs sm:text-sm text-slate-600 leading-relaxed">
        ...
        <div class="glass-panel rounded-xl sm:rounded-2xl lg:rounded-3xl p-4 sm:p-6">
            <div class="flex items-center justify-between mb-4 sm:mb-6">
                <h3 class="text-base sm:text-lg font-semibold text-slate-900">مؤشرات الأداء</h3>
                <span class="text-xs tracking-[0.2em] text-slate-500 bg-slate-50 px-2.5 py-1 rounded-full">تحديث مباشر</span>
            </div>
            <div class="grid gap-3 sm:gap-4 md:grid-cols-3">
                @foreach($resources->take(6) as $resource)
                    ...
                    @endphp
                    <div class="rounded-lg sm:rounded-xl border border-slate-200 bg-white p-3 sm:p-4 text-right hover:bg-slate-50 transition">
        ...
        <div class="glass-panel rounded-xl sm:rounded-2xl lg:rounded-3xl p-4 sm:p-6">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-4 sm:mb-6">
                <h3 class="text-base sm:text-lg font-semibold text-slate-900">آخر التحديثات</h3>
            </div>
            <x-admin.table
            :columns="[
